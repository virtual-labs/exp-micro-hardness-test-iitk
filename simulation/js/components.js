  ...
  const onMouseUpHandler = (event) => {
    if (dragMode == 1) {
      if (mit1 && mit1.isActive() && mit1.isInside(getMouseCoords(event))) {
        mit1.loadSample();
        destroy();
      } if(mit2 && mit2.isActive() && mit2.isInside(getMouseCoords(event))) {
        mit2.loadSample();
        destroy();
      }
      else if(mit3 && mit3.isActive() && mit3.isInside(getMouseCoords(event))) {
        mit3.loadSample();
        destroy();
      }
      ctx.refresh();
...
function MIT2(canvas, ctx) {
  ...
  imgMIT1.src = assetPath + "mit2.png";
  imgMIT1.onload = itemsLoaded++;
  ...
  const paint = () => {
    ...
    if (sampleLoaded) {
      ...
      let x = (xOffset + 1030) * scale;    // sample ke liy
      let y = (yOffset + 520) * scale;
      ctx.drawImage(currentSample, x, y, (currentSample.width * scale) / 3, (currentSample.height * scale) / 3);
    }
  ...
  const isInside = ({ x, y }) => {
    boundary = {
      xmin: (xOffset + 800) * scale,
      xmax: (xOffset + 1300) * scale,
      ymin: (yOffset + 400) * scale,
      ymax: (yOffset + 800) * scale,
    };
...
function MIT3(canvas, ctx) {
  ...
  imgMIT1.src = assetPath + "mit3.png";
  imgMIT1.onload = itemsLoaded++;
  ...
  const paint = () => {
    ...
    if (!sampleLoaded && flashArrow) {
      let _y = y + 110;
      let _x = x -140;                                   // Arrow ke liy
      ctx.fillText("Drag sample here", _x-20, _y + 15);
      ctx.drawImage(arrow, _x + 100, _y, arrow.width / 2, arrow.height / 2);
    }
    ctx.drawImage(imgMIT1, x, y, imgMIT1.width * scale, imgMIT1.height * scale);
    ...
    if (sampleLoaded) {
      ...
      let x = (xOffset + 910) * scale;    // sample ke liy
      let y = (yOffset + 310) * scale;
      ctx.drawImage(currentSample, x, y, (currentSample.width * scale) / 3, (currentSample.height * scale) / 3);
    }
  ...
  const isInside = ({ x, y }) => {
    boundary = {
      xmin: (xOffset + 600) * scale,
      xmax: (xOffset + 1200) * scale,
      ymin: (yOffset + 100) * scale,
